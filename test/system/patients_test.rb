require "application_system_test_case"

class PatientsTest < ApplicationSystemTestCase
  setup do
    @patient = patients(:one)
  end

  test "visiting the index" do
    visit patients_url
    assert_selector "h1", text: "Patients"
  end

  test "creating a Patient" do
    visit patients_url
    click_on "New Patient"

    fill_in "Allergies", with: @patient.allergies
    fill_in "Birthdate", with: @patient.birthdate
    fill_in "Civil state", with: @patient.civil_state
    fill_in "Email", with: @patient.email
    fill_in "Family info", with: @patient.family_info
    fill_in "Hobbies", with: @patient.hobbies
    fill_in "Illness", with: @patient.illness
    fill_in "Job", with: @patient.job
    fill_in "Last name", with: @patient.last_name
    fill_in "Name", with: @patient.name
    fill_in "Phone number", with: @patient.phone_number
    fill_in "Rut", with: @patient.rut
    fill_in "Surgery", with: @patient.surgery
    fill_in "User", with: @patient.user_id
    click_on "Create Patient"

    assert_text "Patient was successfully created"
    click_on "Back"
  end

  test "updating a Patient" do
    visit patients_url
    click_on "Edit", match: :first

    fill_in "Allergies", with: @patient.allergies
    fill_in "Birthdate", with: @patient.birthdate
    fill_in "Civil state", with: @patient.civil_state
    fill_in "Email", with: @patient.email
    fill_in "Family info", with: @patient.family_info
    fill_in "Hobbies", with: @patient.hobbies
    fill_in "Illness", with: @patient.illness
    fill_in "Job", with: @patient.job
    fill_in "Last name", with: @patient.last_name
    fill_in "Name", with: @patient.name
    fill_in "Phone number", with: @patient.phone_number
    fill_in "Rut", with: @patient.rut
    fill_in "Surgery", with: @patient.surgery
    fill_in "User", with: @patient.user_id
    click_on "Update Patient"

    assert_text "Patient was successfully updated"
    click_on "Back"
  end

  test "destroying a Patient" do
    visit patients_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Patient was successfully destroyed"
  end
end
